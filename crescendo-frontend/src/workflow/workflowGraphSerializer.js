...
export function validateGraphForSave(nodes, edges) {
    ...
    for (const edge of edges || []) {
        ...
        if (source?.type === 'branch') {
            ...
            const allowedHandles = source.data?.actionKey === 'logic:if'
                ? ['true', 'false']
                : Array.from({ length: outputCount }, (_, index) => `output_${index}`);
            if (!allowedHandles.includes(sourceHandle)) return 'A branch connection uses an invalid output port.';
        } else if (sourceHandle !== 'out') return 'Only branch nodes can use named output ports.';
...
/** Action keys that require named branch output ports (BranchNode). */
const BRANCH_ACTION_KEYS = new Set(['logic:if', 'logic:switch']);

/**
 * Resolves the React Flow node type for a backend step DTO.
 *
 * - 'trigger' — trigger steps
 * - 'branch'  — logic:if and logic:switch (rendered by BranchNode with named ports)
 * - 'action'  — everything else
 *
 * This is the single source of truth for the type string so AI-generated and
        const pair = `${edge.source}:${edge.target}`;
        if (pairs.has(pair)) return 'This connection already exists.';
        pairs.add(pair);
        // DAG: multiple incoming edges allowed (merge points)
 ...
 *
 * This is the single source of truth for the type string so AI-generated and
 * hand-built branching workflows are treated identically (Scenario D parity).
 *
 * @param {Object|string} stepOrAppKey — backend step DTO or 'trigger'
 * @param {string} [actionKey] — optional action key if not passing a DTO
 * @returns {'trigger'|'branch'|'action'}
 */
export function resolveNodeType(stepOrAppKey, actionKey) {
    if (typeof stepOrAppKey === 'object' && stepOrAppKey !== null) {
        if (stepOrAppKey.type === 'TRIGGER') return 'trigger';
        if (BRANCH_ACTION_KEYS.has(stepOrAppKey.actionKey)) return 'branch';
        return 'action';
    }
    if (stepOrAppKey === 'trigger') return 'trigger';
    if (BRANCH_ACTION_KEYS.has(actionKey)) return 'branch';
    return 'action';
}
