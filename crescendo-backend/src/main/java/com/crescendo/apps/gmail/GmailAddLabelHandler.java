...
/**
 * Adds a label to a Gmail message via users.messages.modify.
 */
@ActionMapping(appKey = "gmail", actionKey = "add-label")
public class GmailAddLabelHandler implements ActionHandler {

    private static final Logger logger = LoggerFactory.getLogger(GmailAddLabelHandler.class);
    private static final String GMAIL_API = "https://gmail.googleapis.com/gmail/v1/users/me/messages/";

    private final RestClient restClient;

    public GmailAddLabelHandler() {
    ...
    public ActionResult execute(ActionContext context) {
        ...
        try {
            ...
            String response = restClient.post()
                    ...
                    .body(Map.of("addLabelIds", List.of(labelId)))
                    .retrieve()
                    .body(String.class);

            Map<String, Object> output = new HashMap<>();
            output.put("provider", "gmail");
            output.put("action", "add-label");
            ...
            logger.error("[gmail] Failed to add label: {}", e.getMessage());
            return ActionResult.failure("Gmail add-label failed: " + e.getMessage());
        }
    }
