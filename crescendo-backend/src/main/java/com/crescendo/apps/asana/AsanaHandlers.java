...
class AsanaCreateTaskHandler implements ActionHandler {
    ...
    public ActionResult execute(ActionContext c) {
        try {
            ...
            String res = SimpleApiSupport
                    .bearer("https://app.asana.com/api/1.0", SimpleApiSupport.cred(c, "accessToken")).post()
                    .uri("/tasks").contentType(MediaType.APPLICATION_JSON).body(Map.of("data", d)).retrieve()
                    .body(String.class);
...
class AsanaListTasksHandler implements ActionHandler {
    ...
    public ActionResult execute(ActionContext c) {
        try {
            String res = SimpleApiSupport
                    .bearer("https://app.asana.com/api/1.0", SimpleApiSupport.cred(c, "accessToken")).get()
                    .uri("/projects/{project}/tasks", SimpleApiSupport.cfg(c, "project")).retrieve().body(String.class);
            return SimpleApiSupport.parsed(m, res);
        } catch (Exception e) {
