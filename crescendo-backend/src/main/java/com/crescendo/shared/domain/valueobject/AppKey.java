...
public record AppKey(String value) {
    ...
    private static final Pattern APP_KEY_PATTERN = Pattern.compile("^[a-z][a-z0-9_-]{1,99}$");
    ...
    public AppKey {
        ...
        if (!APP_KEY_PATTERN.matcher(value).matches()) {
            throw new IllegalArgumentException("App key must start with a letter and contain only lowercase letters, numbers, underscores, and hyphens");
        }
    }
